...
Public Class AdminSubscribers
    Private subsControl As subscriberview
    ...
    Private Sub subscriberview_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            If IsReadOnly = True Then
                If deleteAll IsNot Nothing Then deleteAll.Visible = False
                If btnSelectSubscriber IsNot Nothing Then btnSelectSubscriber.Visible = False

                If btnExport IsNot Nothing Then btnExport.Visible = False

                Dim btnAdd As Control = Me.Controls.Find("btnAddSubscriber", True).FirstOrDefault()
        ...
        Try
            ...
            Using conn As New MySqlConnection(CONNECTION_STRING)
                ...
                If Not String.IsNullOrWhiteSpace(searchText) Then
                    query.Append(" AND (customer_id LIKE @search OR CONCAT(first_name, ' ', last_name) LIKE @search OR DATE_FORMAT(date_installed, '%m/%d/%Y') LIKE @search)")
                End If
            ...
            If IsReadOnly = True Then
                If dgvSubsDeets.Columns.Contains("colEditIcon") Then
                    dgvSubsDeets.Columns("colEditIcon").Visible = False
                End If

                If dgvSubsDeets.Columns.Contains("colDeleteIcon") Then
                    dgvSubsDeets.Columns("colDeleteIcon").Visible = False
                End If

                If dgvSubsDeets.Columns.Contains("colSelect") Then
                    dgvSubsDeets.Columns("colSelect").Visible = False
                End If
            End If
    ...
    '  EDIT BUTTON LOGIC
    ' =========================================================================================
    Private Sub dgvSubsDeets_CellContentClick(sender As Object, e As DataGridViewCellEventArgs)
        If e.RowIndex < 0 Then Exit Sub

        Dim columnName = dgvSubsDeets.Columns(e.ColumnIndex).Name

        If IsReadOnly = True Then
            If columnName = "colEditIcon" OrElse columnName = "colDeleteIcon" OrElse columnName = "colSelect" Then
                Exit Sub
            End If
        End If
        ...
        Dim row = dgvSubsDeets.Rows(e.RowIndex)
        ...
        Select Case columnName
                ...
                If result = DialogResult.Yes Then
                    Try
                        Using conn As New MySqlConnection(CONNECTION_STRING)
                            ...
                            Dim deleteQuery = "UPDATE customer_data SET is_deleted = 1 WHERE customer_id = @customerID"
                            ' ------------------------------
            ...
            Case "colSelect"
                If subscriberID IsNot Nothing Then
                    Dim cell = TryCast(dgvSubsDeets.Rows(e.RowIndex).Cells("colSelect"), DataGridViewCheckBoxCell)
    ...
    Private Sub CBPlanType_SelectedIndexChanged(sender As Object, e As EventArgs)
        LoadSubscriberData()
        LoadChartsData()
        UpdateSubscriberLabels()
    End Sub

    Private Sub CBAccStat_SelectedIndexChanged(sender As Object, e As EventArgs)
        LoadSubscriberData()
        LoadChartsData()
        UpdateSubscriberLabels()
    End Sub

    Private Sub txtSubscriberSearchSA_TextChanged(sender As Object, e As EventArgs)
    ...
    Private Sub btnSubscriberPreviousSA_Click(sender As Object, e As EventArgs)
    ...
    Private Sub btnExport_Click(sender As Object, e As EventArgs)
        Try
            ...
            saveDialog.FileName = "Subscriber_Report_" & Date.Now.ToString("yyyyMMdd_HHmmss")
    ...
    Private Sub btnSelectSubscriber_Click(sender As Object, e As EventArgs)
            ...
            If Not row.IsNewRow Then
                ...
                If shouldSelect Then
                    ...
                    If drv IsNot Nothing Then
                        ...
                        Dim id = Convert.ToInt32(drv("CustomerID"))
    ...
    Private Sub ComboBoxDate_SelectedIndexChanged(sender As Object, e As EventArgs)
        currentPageIndex = 0
        LoadSubscriberData()
        UpdateSubscriberLabels()
        LoadChartsData()
    ...
    Private Sub dgvSubsDeets_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs)
        If e.RowIndex < 0 Then Return

        Dim colName = dgvSubsDeets.Columns(e.ColumnIndex).Name

        ' Alignment Logic
        If colName = "colMonthlyRate" Then
        ...
        If colName = "colDateInstalled" OrElse colName = "DateInstalled" Then
            If e.Value IsNot Nothing Then
                ' If the value is a DateTime object, format it to MM/DD/YYYY
                If TypeOf e.Value Is Date Then
                    Dim dateValue As Date = e.Value
    ...
    Private Sub deleteAll_Click(sender As Object, e As EventArgs)
        ...
        Dim result = MessageBox.Show($"Are you sure you want to delete the {idsToDelete.Count} selected subscriber(s)?",
                                                 "Confirm Bulk Delete",
        ...
        If result = DialogResult.Yes Then
            Using conn As New MySqlConnection(CONNECTION_STRING)
                conn.Open()
                ' Transaction is less critical for a single table update, but good for safety
                Dim transaction = conn.BeginTransaction

                Try
                    Dim paramNames As New List(Of String)
                    For i = 0 To idsToDelete.Count - 1
                        paramNames.Add("@id" & i)
                    Next
                    Dim inClause = String.Join(",", paramNames)

                    ' --- CHANGED TO SOFT DELETE ---
                    ' We ONLY update the customer_data table. We leave payments/service history alone (archived).
                    Dim deleteCustomerQuery = $"UPDATE customer_data SET is_deleted = 1 WHERE customer_id IN ({inClause})"
                    ' ------------------------------
                    ...
                    Using cmdCustomer As New MySqlCommand(deleteCustomerQuery, conn, transaction)
                        ...
                        Dim rowsAffected = cmdCustomer.ExecuteNonQuery
                        transaction.Commit()

                        MessageBox.Show($"{rowsAffected} subscriber(s) deleted successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub dgvSubsDeets_CurrentCellDirtyStateChanged(sender As Object, e As EventArgs)
        If dgvSubsDeets.IsCurrentCellDirty Then
            dgvSubsDeets.CommitEdit(DataGridViewDataErrorContexts.Commit)
        End If
    End Sub

    Private Sub dgvSubsDeets_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs)
