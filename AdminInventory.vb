...
Public Class AdminInventory
    ...
    Private Sub inventoryview_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            If IsReadOnly = True Then
                If btnAddInventory IsNot Nothing Then btnAddInventory.Visible = False

                If btnExport IsNot Nothing Then btnExport.Visible = False

                If deleteAll IsNot Nothing Then deleteAll.Visible = False

                If btnSelectInventory IsNot Nothing Then btnSelectInventory.Visible = False

                ' 5. Hiding columns is handled in PopulateInventoryGrid, 
    ...
    Private Sub PopulateInventoryGrid(dt As DataTable)
        ...
        For Each row As DataRow In dt.Rows
            ...
            If IsReadOnly = True Then
                ' Hide the Edit Button Column
                If InventoryDetailsDVG.Columns.Contains("colEdit") Then
                    InventoryDetailsDVG.Columns("colEdit").Visible = False
                End If

                ' Hide the Delete Button Column
                If InventoryDetailsDVG.Columns.Contains("colDelete") Then
                    InventoryDetailsDVG.Columns("colDelete").Visible = False
                End If

                ' Hide the Checkbox Column (Since they can't bulk delete)
                If InventoryDetailsDVG.Columns.Contains("colCheckBox") Then
                    InventoryDetailsDVG.Columns("colCheckBox").Visible = False
                End If
            End If
        Next
    End Sub
    ...
    Private Sub InventoryDetailsDVG_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles InventoryDetailsDVG.CellClick
        ...
        If IsReadOnly = True Then
            Dim colName As String = InventoryDetailsDVG.Columns(e.ColumnIndex).Name
            If colName = "colEdit" Or colName = "colDelete" Or colName = "colCheckBox" Then Exit Sub
        End If
    ...
    Private Sub btnDeploy_Click(sender As Object, e As EventArgs)
        ' 1. Gather Selected IDs
        Dim itemsToProcess = selectedInventoryIds.ToList

        If itemsToProcess.Count = 0 AndAlso InventoryDetailsDVG.SelectedRows.Count > 0 Then
            Dim id = InventoryDetailsDVG.SelectedRows(0).Cells("ItemID").Value.ToString
